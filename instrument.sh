...
(
	...
	"$TARGET/build.sh"
    $CXX $CXXFLAGS -std=c++11 -c "$FUZZER/repo/driver.cpp" -fPIC -o "$OUT/driver.o"
)
...
(
    ...
    for p in "${PROGRAMS[@]}"; do (
		...
		echo "[+] precondInfer"
		$OUT/precondInfer $p.bc --target-file=$OUT/cstest.txt --join-bound=1 > log_precond.txt 2>&1
		
		echo "[+] Ins"
		$OUT/Ins -output=$folder/fuzz.bc -afl -res=$folder -log=log_Ins.txt -load=$folder/range_res.txt $folder/transed.bc
